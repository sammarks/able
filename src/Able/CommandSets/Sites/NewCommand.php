...
class NewCommand extends BaseCommand
{
	...
	protected function configure()
	{
		...
		$this
			...
			->addOption('site-address',
				...
				null)
			->addOption('skip-virtualhost',
				null,
				InputOption::VALUE_NONE,
				'Whether or not to skip the creation of a virtualhost.',
				null)
			->addOption('skip-install',
				null,
				InputOption::VALUE_NONE,
				'Skip the installation of the website and the creation of the database.',
				null)
			->addOption('skip-files',
				null,
				InputOption::VALUE_NONE,
				'Skip the setup of the Drupal installation.',
				null);
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if (!$input->getOption('skip-files')) {
			...
			$success = $this->exec("chmod -R 775 {$site_path}", false);
			$success_two = $this->exec("chown -R www-data:www-data {$site_path}", false);
			if (!$success || !$success_two) {
				$this->log('There appears to have been an error updating the permissions for the site.' .
					' Fear not! It\'s probably because you\'re running something like Vagrant. Ignoring.', 'yellow');
			}
		...
		if (!$input->getOption('skip-files')) {
			...
			if (!file_exists("{$site_path}/sites/default/settings.php") &&
				file_exists("{$site_path}/sites/default/default.settings.php")) {
				copy("{$site_path}/sites/default/default.settings.php", "{$site_path}/sites/default/settings.php");
			}
			...
			$this->log('Fixing permissions on settings.php', 'white', self::DEBUG_VERBOSE);
			$this->exec("chmod 777 {$site_path}/sites/default/settings.php", false);
			file_put_contents("{$site_path}/sites/default/settings.php", $settings);
			$this->exec("chmod 644 {$site_path}/sites/default/settings.php", false);
		...
		if (!$input->getOption('skip-virtualhost')) {
			...
			if ($web_server == 'nginx') {
				$this->log('Creating nginx Log Folders', 'white', self::DEBUG_VERBOSE);
